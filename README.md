...
YouTube 링크로 음악을 백그라운드에서 재생할 수 있는 Flutter 앱

---

## 개요

**Music On**은 YouTube URL을 입력하면 오디오만 추출해 백그라운드에서 재생하는 음악 플레이어 앱입니다. 즐겨찾기와 커스텀 플레이리스트를 지원하며, 화면이 꺼져 있어도 계속 재생됩니다.
...
| 기능 | 설명 |
|------|------|
| YouTube 재생 | YouTube URL 입력 후 오디오 백그라운드 재생 |
...
dependencies:
  # YouTube 오디오 추출
  youtube_explode_dart: ^2.2.0      # YouTube 메타데이터 & 스트림 URL 추출
...
│   │   └── app_colors.dart
│   └── utils/
│       └── youtube_utils.dart      # URL 파싱, ID 추출
│
├── data/
│   ├── models/
...
│   └── youtube_service.dart        # youtube_explode로 스트림 URL 획득
│
├── providers/
...
사용자 URL 입력
    ↓
youtube_explode_dart → 오디오 스트림 URL 추출
    ↓
...
3. **YouTubeService** — URL → 오디오 스트림 URL
